...
def main():
    ...
    parser.add_argument("--urdf",
                        ...
                        default="")
    parser.add_argument("--initial-base-pos",
                        type=float,
                        nargs=3,
                        help="Initial base position (x y z)",
                        default=[0.0, 0.0, 0.75])
    parser.add_argument("--final-base-pos",
                        type=float,
                        nargs=3,
                        help="Final base position (x y z)",
                        default=[0.0, 0.0, 0.35])
    parser.add_argument("--step-size", type=float, help="Step size for interpolation", default=0.05)
    ...
    progress_handle = server.gui.add_slider("Progress",
                                            min=0,
                                            max=num_steps,
                                            step=1,
                                            initial_value=0,
        ...
        for i, base_pos in enumerate(base_positions):
            ...
            base_pose_SE3 = jaxlie.SE3.from_rotation_and_translation(
                jaxlie.SO3.from_quaternion_xyzw(
                    jnp.array([
                        base_wxyz_result[1], base_wxyz_result[2], base_wxyz_result[3],
                        base_wxyz_result[0]
